...
for problema in range(1,12):
    for cont in range(10, 100, 10):
        ...
        my_data1 = numpy.genfromtxt(Best % (problema, cont), delimiter='', dtype=float)


        fig =plt.figure(1)
        plt.subplot(221)
        plt.plot(my_data3,my_data1,'go')

        plt.subplot(222)
        plt.plot(my_data3,my_data1,'bo')

        plt.subplot(223)
        plt.plot(my_data3,my_data1,'rs')
        plt.close(fig)
