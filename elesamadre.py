...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, problema,cont,run,Funcion,stats=None,
    ...
    outfile = open('./Results/Problem%d/Res%d/BestSizAVGe%d_%d.txt'%(problema,Funcion,cont,run),'ab')
    outfile.write('\n%s;%s;%s'%(0,len(best_ind),size_avgs[-1]))
    ...
    for gen in range(1, ngen+1):
        ...
        if verbose:
            ...
            size_avgs = logbook.chapters["size"].select("avg")
        best_ind=best_pop(population)
        outfile.write('\n%s;%s;%s'%(gen,len(best_ind),size_avgs[-1]))

    return population, logbook, size_avgs
